...
# ==========================================================
# Part 1: Attention Mechanisms (CBAM, etc.)
# ==========================================================
...
# ==========================================================
# Part 2: Building Blocks (ResBlock, ASPP)
# ==========================================================
...
class ResnetBlock3D(nn.Module):
    ...
    def build_conv_block(self, dim, norm_layer, use_dropout, use_bias, dilation):
        conv_block = []
        p = dilation 
        conv_block += [nn.Conv3d(dim, dim, kernel_size=3, padding=p, dilation=dilation, bias=use_bias, padding_mode='reflect'),
                       norm_layer(dim),
                       nn.ReLU(False)]
        if use_dropout:
...
class ASPP3D(nn.Module):
    def __init__(self, in_channels, out_channels, norm_layer=nn.BatchNorm3d):
        ...
        self.branch2 = nn.Sequential(
            nn.Conv3d(in_channels, out_channels, kernel_size=3, padding=2, dilation=2, bias=False, padding_mode='reflect'),
            norm_layer(out_channels),
            nn.LeakyReLU(0.2, True)
        )
        self.branch3 = nn.Sequential(
            nn.Conv3d(in_channels, out_channels, kernel_size=3, padding=3, dilation=3, bias=False, padding_mode='reflect'),
            norm_layer(out_channels),
            nn.LeakyReLU(0.2, True)
        )
        self.branch5_conv = nn.Sequential(
            nn.Conv3d(in_channels, out_channels, kernel_size=1, bias=False),
            # norm_layer(out_channels),
            nn.Conv3d(in_channels, out_channels, kernel_size=1, bias=True), # 加上 bias=True 补偿
            nn.LeakyReLU(0.2, True)
        )
